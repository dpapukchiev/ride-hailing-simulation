...
fn test_sustained_load() {
    ...
    println!("Sustained load test: {} events in {:.2}s ({:.0} events/sec)", 
             events, duration.as_secs_f64(), events_per_sec);
    
    // Assert minimum performance threshold
    assert!(events_per_sec > 1000.0, 
            "Should process >1000 events/sec, got {:.0}", events_per_sec);
}
...
fn test_peak_load() {
    ...
    let events_per_sec = events as f64 / duration.as_secs_f64();
    println!("Peak load test: {} events in {:.2}s ({:.0} events/sec)", 
             events, duration.as_secs_f64(), events_per_sec);
    
    // Should handle peak load gracefully
    assert!(events_per_sec > 500.0, 
            "Should process >500 events/sec under peak load, got {:.0}", events_per_sec);
}
...
fn test_long_running() {
    ...
    let events_per_sec = events as f64 / duration.as_secs_f64();
    println!("Long-running test: {} events in {:.2}s ({:.0} events/sec)", 
             events, duration.as_secs_f64(), events_per_sec);
    
    // Should maintain consistent performance
    assert!(events_per_sec > 500.0, 
            "Should process >500 events/sec in long-running scenario, got {:.0}", events_per_sec);
}
