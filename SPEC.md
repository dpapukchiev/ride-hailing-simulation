...
- **Serverless sweep deployment path (AWS)**: API Gateway ingress invokes a unified runtime Lambda that validates requests, computes deterministic shard plans, and enqueues shard messages to SQS for distributed execution. The same runtime handles queue-driven worker processing and persists outcomes to partitioned S3 datasets for Athena analytics. Runtime contracts/sharding primitives are exposed through the runtime boundary in `sim_serverless_sweep_lambda`; Terraform in `infra/aws_serverless_sweep/` remains infrastructure wiring.
